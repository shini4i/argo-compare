...
Which repositories a git generator may read, which generated Applications are compared, and what is a skip versus a hard error are laid out in [Setting up ApplicationSets](applicationset-setup.md#2-applicationset-in-another-repository), with a worked layout under [`examples/applicationset/cross-repo/`](../examples/applicationset/cross-repo).
...
- Each source of a multi-source Application renders in a directory of its own, keyed to its `repoURL`, its `chart` or `path`, and its `helm.releaseName`, so two sources sharing a chart name do not overwrite each other. That key is what the rendered paths in the diff carry, and it lets one chart render twice under two release names. Two sources sharing all of it are rejected: nothing else — `targetRevision` or values — tells them apart on disk.
...
- Multi-source `ref` sources are supported and follow the same `$values` resolution and credential-scoping rules as templates do; see [Multi-source templates and `ref` sources](applicationsets.md#multi-source-templates-and-ref-sources).
...
- An anchored ApplicationSet is subject to every limit in [ApplicationSets](applicationsets.md), plus the repository rules in [Setting up ApplicationSets](applicationset-setup.md#2-applicationset-in-another-repository).
