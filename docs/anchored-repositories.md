...
The manifest is read once, at the anchor's branch tip: an anchor exists because the *chart* changed, not the manifest.
...
A git generator may name either repository, and which one it names decides how expansion runs:

- **The repository being compared.** Each leg expands the manifest against its own tree, so a directory the generator picks up — or a file it takes parameters from — is read from this branch for one leg and from the merge-base for the other. A directory the change adds or drops is reported as an added or removed Application.
- **The repository the anchor fetched the manifest from.** The generator is expanded against that repository's clone, which is already in memory from the fetch. Both legs therefore generate the same Applications: the pull request does not touch that repository, so the diff comes from what those Applications read here — typically a `ref` source's values files. Its `revision` must be `HEAD` or the branch the anchor read, since that is the only revision the fetch holds.

A generator naming a third repository is a hard error rather than a skip, and so is an ApplicationSet whose generators name both repositories at once — expansion takes one tree, so serving both would drop a generator's Applications without saying so. An anchor is an explicit pointer, so ignoring one would leave the change it names uncompared with nothing said.

Only the generated Applications a change to this repository can affect are compared. One with a `spec.source.path` belonging to a different repository is skipped with the reason named — rendering it would diff this repository's content against another repository's Application. A registry chart is skipped too, unless it takes its values from a multi-source `ref` source pointing at this repository: then a values change here does change the render, so it is compared. If no generated Application qualifies, that is an error for the same reason a foreign generator is.
...
- A multi-source Application must use one kind consistently; mixing `chart` and `path` entries is rejected. A `ref` source is exempt — it renders nothing, so it neither counts as a kind nor makes the Application mixed.
- An anchored Application rendering a registry chart is accepted when its values come from a `ref` source pointing at this repository — the change under the anchor does reach the render. Without such a ref it is still rejected as not path-based.
- Multi-source `ref` sources are supported: a source carrying `ref: values` (and no `chart`) is addressed as `$values/path/to/file.yaml` from a sibling source's `helm.valueFiles`. When its `repoURL` matches this repository, each leg reads its own revision of the file — the working tree for the source leg, the merge-base for the destination leg — so a values change shows in the diff; its `targetRevision` is ignored, exactly as a path-based source's is. When it names another repository, the file is read from a shallow clone at `targetRevision` — each leg at the revision its own side of the manifest names, so bumping that `targetRevision` in the pull request does show up, while leaving it alone means the external repository contributes nothing to the diff. `ARGO_COMPARE_GIT_USERNAME` / `ARGO_COMPARE_GIT_TOKEN` are sent **only** to a repository on the exact same HTTPS endpoint as `origin` — matching scheme, host and port — because a `repoURL` comes from the Application manifest, which a pull request author controls, so authenticating against an arbitrary host would hand the CI token to whatever repository a branch pointed at. A values repository on another host must therefore be readable anonymously, and an `ssh://` ref source authenticates through the SSH agent instead, the same way an anchor's cross-repo clone does. A remote ref source must name a branch or a tag; a commit SHA is rejected with that reason. A `$name` no source declares, and a referenced file that is absent, are both hard errors rather than a silently thinner diff.
- Glob patterns in `helm.valueFiles` (e.g. `$values/envs/*.yaml`) are not expanded; each entry must name one file.
...
- An anchored ApplicationSet's git generators must all read the same repository — either the one being compared or the one the anchor fetched the manifest from. Naming a third repository, or both at once, is a hard error. A generator reading the anchored repository must leave `revision` at `HEAD` or set it to the branch the anchor read, since that is the only revision the fetch holds.
